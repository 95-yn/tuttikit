...
const PORTS = { web: 3000, server: 3001 };
...
async function freePort(label, port) {
  ...
  if (conflicts.length) {
    console.error(
      `${C.red}✗ 端口 ${port} (${label}) 被「非本项目进程」占用：${C.reset}`,
    );
    for (const { pid, cmd } of conflicts) {
      console.error(`  ${C.dim}PID ${pid}:${C.reset} ${cmd.slice(0, 120)}`);
    }
    console.error(
      `${C.yellow}请手动停掉这些进程，或修改 PORT/web 端口后重试。${C.reset}`,
    );
    return false;
  }
  ...
  if (ownPids.length) {
    console.log(
      `${C.yellow}⚠${C.reset}  端口 ${port} (${label}) 被上次未退干净的 dev 进程占用，清理：`,
    );
